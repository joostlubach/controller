...
export default function mount(app, controllers) {
...
function mountController(app, Controller) {
    ...
    if (isFunction(Controller.mount)) {
        Controller.mount(app);
    }
}
function mountAction(app, Controller, action, middleware, base) {
    ...
    app[method](fullPath, createAction(Controller, action, middleware));
}
function createAction(Controller, action, middleware) {
...
function convertParams(action, params) {
...
function wrapInHTTPError(error, action) {
...
function callMiddlewareChain(middleware, action, request, response) {
    const todo = [...middleware];
    return new Promise((resolve, reject) => {
        const nextHandler = (error = null) => {
